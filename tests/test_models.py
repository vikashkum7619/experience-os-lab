...
def test_applicability_result_can_be_apply() -> None:
    result = ApplicabilityResult(
        status=ApplicabilityStatus.APPLY,
        matched_conditions=["traveler_type", "checked_baggage"],
        reason="All required experience conditions match.",
    )
...
def test_applicability_result_can_be_uncertain() -> None:
    result = ApplicabilityResult(
        status=ApplicabilityStatus.UNCERTAIN,
        matched_conditions=["traveler_type"],
        uncertain_conditions=["urgency"],
        reason="The experience matches the traveler type but urgency is unknown.",
    )

    assert result.status == ApplicabilityStatus.UNCERTAIN
    assert result.uncertain_conditions == ["urgency"]


def test_applicability_result_can_be_reject() -> None:
    result = ApplicabilityResult(
        status=ApplicabilityStatus.REJECT,
        mismatched_conditions=["traveler_type"],
        reason="The traveler type does not match.",
    )

    assert result.status == ApplicabilityStatus.REJECT
    assert result.mismatched_conditions == ["traveler_type"]